...
public class SortIntegersByTheNumberOfBits {
    ...
    public static int[] sortByBits(int[] arr) {
        for (int i = 1; i < arr.length; i++) {
            ...
            int index = i - 1;
            while(index >= 0 && bits <= Integer.bitCount(arr[index])) {
                if((bits == Integer.bitCount(arr[index]) && temp < arr[index]) || bits < Integer.bitCount(arr[index])) {
                    arr[index + 1] = arr[index];
                } else if(bits == Integer.bitCount(arr[index])) {
                    break;
                }
                index--;
            }
    ...
    public static void main(String[] args) {
        Utils.printArrays(sortByBits(new int[]{1024,512,256,128,64,32,16,8,4,2,1}));
    }
}
